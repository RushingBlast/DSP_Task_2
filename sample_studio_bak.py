...
# Connections
    ...
    def open_sig_file(self):
        try:
            files_name = QFileDialog.getOpenFileName(self, 'Open only txt or CSV or xls', os.getenv('HOME'), "(*.csv *.txt *.xls)")
            path = files_name[0]
    ...
    def plot_sig_on_plot_widget_component(self):
        ...
        freq = Signal_Composer.return_zero_at_emptyString(freq)
        amp = Signal_Composer.return_zero_at_emptyString(amp)
        phase = Signal_Composer.return_zero_at_emptyString(phase)
    ...
    def plot_resultant_sig_on_mainGraph(self):
        self.plotOnMain(Sinusoidals.resultant_sig[0], Sinusoidals.resultant_sig[1], "Resultant Signal from Composer")
        Signal_Composer.export_resultant_as_csv(Signal_Composer.exported_signal_index)
    ...
    def Renew_Intr(self, Freq):
        ...
        self.Dotted_plot = self.gui.plot_widget_main_signal.plot(self.Signal_test.Time_Intrv, self.Signal_test.Samples, pen=pg.mkPen("k", width=0, alpha=0.1), symbol='+')

        self.gui.plot_widget_restored_signal.clear()
        self.gui.plot_widget_restored_signal.plot(self.Signal_test.TimeAxis, Signal_Composer.interpolate_f, pen="b")
        self.gui.label_constructed_signal.setText("[Sampling frq. :"+str(Freq)+"]")

        # Calculate the difference and plot it
        difference = np.subtract(self.Signal_test.amplitude, Signal_Composer.interpolate_f)
...
class Signal_Class(object):
    ...
    def get_Interpolation(self, freq):
        ...
        Signal_Composer.interpolate_f = 0  # class Variable
        ...
        for index in range(0, num_points):  # interpolation with sinc function
            Signal_Composer.interpolate_f += self.Samples[index] * np.sinc((np.array(self.TimeAxis) - Ts * index) / Ts)
        # time_interval = 1.0 / freq
        # self.Time_Intrv = np.arange(self.time[0], self.time[-1], time_interval)
        # Signal_Composer.interpolate_f = np.interp(self.Time_Intrv, self.time, self.amplitude)
        
    # This is a comment to test out live capabilities
    def get_maxFreq(self):
        for index in range(1, len(self.Freq) // 2):
            if (1e-1 < self.Power[index] - self.Power[index - 1]) and (1e-1 < self.Power[index] - self.Power[index + 1]):
...
#     def get_Interpolation(self, target_frequency):
#         # Calculate the time interval based on the target frequency
#         time_interval = 1.0 / target_frequency

#         # Create the time axis for the interpolated signal
#         self.Time_Intrv = np.arange(self.time[0], self.time[-1], time_interval)

#         # Interpolate the signal using np.interp
#         Signal_Composer.interpolate_f = np.interp(self.Time_Intrv, self.time, self.amplitude)
